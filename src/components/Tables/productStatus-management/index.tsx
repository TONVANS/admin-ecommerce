...
import { useProductStatusStore } from "@/store/product/productStatusStore";
...
export default function ProductStatusManagement() {
  const {
    statuses,
    loading,
    error,
  ...
  const [newStatus, setNewStatus] = useState<{ name: string; code: string }>({
    name: "",
    code: "",
  });
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      if (isEdit && editingStatus) {
        await updateStatus(editingStatus.id, newStatus);
        toast.success('ອັບເດດສະຖານະສຳເລັດ'); // ໃຊ້ Toast
      } else {
        await createStatus(newStatus);
        toast.success('ເພີ່ມສະຖານະໃໝ່ສຳເລັດ'); // ໃຊ້ Toast
      }
      await getStatuses();
  ...
  const handleDelete = async (id: number) => {
    if (confirm("ທ່ານແນ່ໃຈບໍ່ວ່າຕ້ອງການລົບສະຖານະນີ້?")) {
      try {
        await deleteStatus(id);
        await getStatuses();
        toast.success('ລົບສະຖານະສຳເລັດ'); // ໃຊ້ Toast
      } catch (err: any) {
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 dark:border-gray-700">
        <Table className="min-w-full">
          <TableHeader className="bg-gray-50 dark:bg-dark-3">
            <TableRow>
              <TableHead className="pl-6 pr-4 py-4 text-sm font-semibold text-gray-600 dark:text-gray-400">ຊື່ສະຖານະ</TableHead>
            ...
            {filteredStatuses.map((status, index) => (
              ...
              >
                <TableCell className="pl-6 pr-4 py-4">
                  <div className="font-semibold text-gray-900 dark:text-white">{status.name}</div>
                </TableCell>
                <TableCell className="px-4 py-4">
                  <div className="text-sm text-gray-600 dark:text-gray-400">{status.code ?? 'N/A'}</div>
                </TableCell>
        ...
        {filteredStatuses.length === 0 && (
          <div className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="text-gray-400 dark:text-gray-500">ບໍ່ພົບຂໍ້ມູນສະຖານະທີ່ກົງກັບການຄົ້ນຫາ</p>
          </div>
        )}
      </div>
                ...
                <DialogPanel className="mx-auto w-full max-w-lg rounded-2xl bg-gradient-to-br from-white to-gray-50 dark:from-dark-2 dark:to-gray-900 p-8 shadow-2xl transform transition-all duration-300 border border-gray-200 dark:border-gray-700">
                  ...
                  <div className="relative text-center mb-8">
                    ...
                    <DialogTitle className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                      {isEdit ? 'ແກ້ໄຂສະຖານະ' : 'ເພີ່ມສະຖານະໃໝ່'}
                    </DialogTitle>
                    <p className="text-sm text-gray-600 dark:text-gray-400 font-medium px-4">
                      {isEdit ? 'ແກ້ໄຂຂໍ້ມູນສະຖານະສິນຄ້າ' : 'ກະລຸນາປ້ອນຂໍ້ມູນສະຖານະໃໝ່'}
                    </p>
                  </div>
